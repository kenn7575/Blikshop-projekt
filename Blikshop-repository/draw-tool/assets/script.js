...
var list1 = [];
//function that listens for click event, gets the svg point of click and then create path
svg.addEventListener("click", (event) => {
  listOfPoints.push(calculateSVGPoints(event.clientX, event.clientY));
  createSVGPath(listOfPoints);
  createSVGText(listOfPoints);
  ...
  if (lastPointIndex >= 1) {
    ...
    console.log("vectors" + listOfVectors);
...
const createSVGText = (input) => {
  var modifyedList = angleOfVectors;

  //modifyedList.push("");
  //modifyedList.unshift("");
  console.log(modifyedList);
  var count = 0;
  document.querySelector(".Layer-2").innerHTML = "";
  input.forEach((element) => {
    const textLayer = document.querySelector(".Layer-2");
    ...
    textLayer.insertAdjacentHTML("beforeend", content);
  });
};
